...
const { UntarFileStream } = await import(
  chrome.runtime.getURL('UntarFileStream.js')
);

let tarFileStream = new UntarFileStream(decompressedBuffer);
while (tarFileStream.hasNext()) {
  const file = tarFileStream.next();
  console.log(file);
  if (file.name === 'node-v17.9.0-linux-x64/bin/node') {
    break;
  }
}
*/
